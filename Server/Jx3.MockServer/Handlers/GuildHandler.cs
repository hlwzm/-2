...
public class GuildHandler : HandlerBase, IHandler
{
    ...
    byte[] HandleApply(BinaryReader br, uint seq) { var pid = br.ReadUInt64(); var gid = br.ReadUInt64(); GuildStore.Instance.AddApply(gid, pid); return BuildResponse((uint)MsgId.CSGuildApply, seq, w => w.Write(0)); }

    byte[] HandleLeave(BinaryReader br, uint seq) { var pid = br.ReadUInt64(); var g = GuildStore.Instance.GetByPlayer(pid); if (g != null) GuildStore.Instance.LeaveGuild(g.GuildId, pid); return BuildResponse((uint)MsgId.CSGuildLeave, seq, w => w.Write(0)); }
